...
(function () {
  ...
  if (!card || !button || !window.THINKERS || !window.THINKERS.length) return;
  ...
  function render(t) {
    ...
    card.innerHTML =
      ...
      '<div class="draw__text">' +
        ...
        '<h3 class="draw__name">' + esc(t.n) + (t.d ? ' <span class="draw__dates">' + esc(t.d) + '</span>' : '') + '</h3>' +
    ...
    var again = card.querySelector('.draw__again');
    if (again) again.addEventListener('click', function () { render(pick()); });
    loadOne(card.querySelector('.portrait[data-wiki]'));
  }

  button.addEventListener('click', function () { render(pick()); });
})();
